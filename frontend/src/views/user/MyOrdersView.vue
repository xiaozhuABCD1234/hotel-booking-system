...
<template>
  <div class="container mx-auto max-w-5xl">
    <h1 class="mb-6 text-2xl font-semibold text-gray-900">我的订单</h1>
    ...
    >
      ...
      <Button class="bg-blue-900 hover:bg-blue-800" @click="router.push('/')">
          ...
          <Table>
            ...
            <TableBody>
              <TableRow v-for="order in orders" :key="order.id">
                ...
                <TableCell>
                  <Badge :class="getStatusBadge(order.status).class">
    ...
    >
      <Button
        ...
        @click="
          orderStore.fetchMyOrders(
            authStore.user!.id,
            (orderStore.pagination?.currentPage ?? 1) - 1,
          )
        "
      >
        上一页
      ...
      <span class="text-sm text-gray-500">
        {{ orderStore.pagination.currentPage }} /
  ...
  <Dialog
    :open="dialogOrderId !== null"
    @update:open="
      (v) => {
        if (!v) dialogOrderId = null;
      }
    "
  >
    <DialogContent>
      ...
      <DialogFooter>
        <Button variant="outline" @click="dialogOrderId = null"> 返回 </Button>
        <Button
          variant="destructive"
