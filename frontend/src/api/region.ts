...
export const regionApi = {
  ...
  listByParent(parentId: number) {
    return api.get<ApiResponse<Region[]>>("/regions/by-parent", {
...
export const reportApi = {
  ...
  roomDetailsByHotel(hotelId: string) {
    return api.get<ApiResponse>("/reports/room-details/by-hotel", {
  ...
  topGuests(limit = 10) {
    return api.get<ApiResponse<GuestBookingStats[]>>(
      "/reports/guest-stats/top",
      { params: { limit } },
    );
  },
};
